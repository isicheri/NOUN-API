...
import { Order, PaymentStatus, Prisma, RequestStatus } from '@prisma/client';
...
export class PaymentService {
  ...
  async handlePaystackWebhook(headers: any, bodyOrRawBody: any, isRaw = false) {
    ...
    let order:
      | (Order & {
          user: { email: string };
          items: { pdf: { id: string; fileKey: string; title: string } }[];
        })
      | undefined;
    ...
    if (order && order.user?.email) {
      ...
      const downloadLinks = await Promise.all(
        order.items.map(async (item) => {
          const signedUrl = await this.pdfService.getSignedUrl(
            item.pdf.fileKey,
          );

import { Body, Controller,Post,Patch, UseGuards, Req, Get, Query, Param, Delete} from "@nestjs/common";
...
markAsPaid(@Body() dto: MarkAsPaidDto) {
  ...
  async findAll(@Query() query: GetCourseSummaryRequestDto) {
   ...
   @UseGuards(JwtAuthGuard,RolesGuard)
   @Roles(Role.ADMIN)
  async viewCourseSummaryRequestInfo(
  ...
  async updateCourseSummaryRequestStatus(
    @Param("requestId") id: string,
    @Param("status",new ParseEnumPipe(RequestStatus,"status")) status: RequestStatus
  ) {

...
import { $Enums, Prisma, RequestStatus } from '@prisma/client';
...
export class CourseSummaryService {
  ...
  async getCourseSummaryRequests(query: GetCourseSummaryRequestDto) {
    ...
    const where: Prisma.CourseSummaryRequestWhereInput = {};

    if (status) {
      where.status = status as $Enums.RequestStatus;
    }
  ...
  async viewCourseRequestInfo(requestId: string) {
    const request = await this.prisma.courseSummaryRequest.findFirst({
      where: { id: requestId },
    });

    if (!request)
      throw new BadRequestException('Course Summary Request Not found!');
    return request;
  }
  ...
  async updateCourseRequestStatus(requestId: string, status: RequestStatus) {
    ...
    if (!request)
      throw new BadRequestException('Course Summary Request Not found!');
    await this.prisma.courseSummaryRequest.update({
      where: { id: requestId },
      data: {
        status,
      },
